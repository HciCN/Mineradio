...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/anysource/search') {
    try {
      const kw = url.searchParams.get('keywords') || '';
      const source = url.searchParams.get('source') || 'kuwo';
      ...
      if (source === 'all') {
        ...
        results.forEach(result => {
          ...
          (result.value || []).forEach(song => {
            const key = [song.anySource || '', song.id || '', song.name || '', song.artist || ''].join('|');
        ...
        songs = await searchAnySource(kw, { source, limit });
      }
      sendJSON(res, { provider: 'anysource', adapter: 'gdstudio', source, songs });
    } catch (err) {
      console.error('[AnySourceSearch]', err);
      sendJSON(res, { provider: 'anysource', adapter: 'gdstudio', error: err.message, songs: [] }, 500);
    }
    return;
  }

  if (pn === '/api/anysource/song/url') {
    try {
      const body = req.method === 'POST' ? await readRequestBody(req) : {};
      const song = body.song || {
        provider: 'anysource',
        anySourceProvider: url.searchParams.get('adapter') || 'gdstudio',
        anySource: url.searchParams.get('source') || '',
        id: url.searchParams.get('id') || '',
        name: url.searchParams.get('name') || '',
        artist: url.searchParams.get('artist') || '',
        album: url.searchParams.get('album') || '',
      };
      const quality = body.quality || url.searchParams.get('quality') || '';
      const info = await getAnySourceSongUrl(song, quality);
      sendJSON(res, info);
    } catch (err) {
      console.error('[AnySourceSongUrl]', err);
      sendJSON(res, { provider: 'anysource', url: '', playable: false, error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/anysource/lyric') {
    try {
      const body = req.method === 'POST' ? await readRequestBody(req) : {};
      const song = body.song || {
        provider: 'anysource',
      ...
      sendJSON(res, await getAnySourceLyric(song));
    } catch (err) {
